...
export default function QuotationDownload({ data, language }: Props) {
  ...
  return (
    <PDFDownloadLink
      ...
      fileName={`Quotation-${data.clientName || 'Client'}-${new Date().toISOString().split('T')[0]}.pdf`}
      className="w-full block"
    >
      {({ loading }) => (
        <button
          disabled={loading}
          className={`w-full py-4 rounded-lg font-bold text-center transition-colors ${
            loading ? 'bg-zinc-800 text-zinc-400' : 'bg-white text-black hover:bg-zinc-200'
          }`}
        >
          {loading 
