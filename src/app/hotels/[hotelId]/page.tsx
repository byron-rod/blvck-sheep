...
export default function HotelDetailPage({ params }: Props) {
  ...
  const whatsappUrl = `https://wa.me/50255116881?text=${encodeURIComponent("Hello!, I am interested in booking a shuttle to " + hotel.name + "!")}`;

  const handleShare = async () => {
    const shareData = {
      title: `Blvck Sheep: ${hotel.name}`,
      text: `Discover ${hotel.name} in ${hotel.location}!`,
      ...
      <section className="relative h-[85vh] w-full flex items-end pb-24 px-6 md:px-12 lg:px-24">
        ...
        <div className="relative z-10 w-full">
          ...
          >
            ...
            <p className="text-lg md:text-2xl text-zinc-300 font-light tracking-wide mix-blend-screen mb-6">
              {hotel.location}
            </p>
          </motion.div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            <Link
              ...
              className="w-full sm:w-auto border border-zinc-700 bg-transparent text-white px-6 py-3 rounded-none text-sm uppercase tracking-[0.2em] font-semibold hover:bg-zinc-900 transition-colors text-center"
            >
              Book Shuttle
            </Link>
            <Link
